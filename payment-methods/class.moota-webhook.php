...
class Moota_Webhook {
	...
	private static $callback_name = 'mootapay-callback';
	...
	public static function _endpoint_handler() {
		global $wp_query;
		...
		if (
			...
			if ( $http_signature && $_SERVER['REQUEST_METHOD'] == 'POST' ) {
				...
				$secret   = get_option('mootapay_access_token');

				$status_paid = get_option("success_status");
