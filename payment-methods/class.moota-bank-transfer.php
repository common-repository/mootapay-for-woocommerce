	...
	public function init_form_fields() {
		$this->form_fields = array(
			'enabled' 	=> array(
				...
				'type'    		=> 'checkbox',
				'label'   		=> __( 'Aktifkan Mootapay Virtual Account', 'woocommerce-gateway-moota' ),
				'default' 		=> 'yes'
			),
	...
	public function bank_lists_bank( $html, $k, $v, $object ) {
		...
		<?php if ( is_array( $banks ) ) : ?>
			<?php foreach ( $banks as $item ) :
				$field_key_bank = $item->payment_method_id;
				$checked = $this->bank_lists_checked( $k, $field_key_bank, $item->payment_method_id );
				?>
                <tr valign="top">
                    ...
                    <td class="forminp">
                        <fieldset>
                            ...
                            <input type="checkbox" name="<?php echo esc_attr($field_key); ?>[<?php echo esc_attr($field_key_bank); ?>]"
                                   id="<?php echo esc_attr($field_key) . '_' . esc_attr($item->payment_method_id); ?>"
	...
	private function bank_selection( $bank_id ) {
        ...
        if ( ! empty($this->all_banks) ) {
            foreach ($this->all_banks as $bank) {
                if ( $bank_id == $bank->payment_method_id ) {
                    return $bank;
                }
            }
        }
        ...
        $banks = $this->settings['bank_lists'];
		 ...
		 <?php if ( ! empty( $banks ) ) :
             foreach ( $banks as $item ) :
                 ...
                 ?>
                 <li>
                     <label for="bank-transfer-<?php echo esc_attr($bank_selection->name); ?> bank-id-<?php echo esc_attr($item); ?>">
                     <input id="bank-transfer-bank-id-<?php echo esc_attr($item); ?>" name="channels" type="radio"
                     value="<?php echo esc_attr($item); ?>">
                     <span><img src="<?php echo esc_attr($bank_selection->icon);?>" alt="<?php echo esc_attr($bank_selection->name); ?>"></span>
                     <span class="moota-bank-account"><?php echo esc_attr($bank_selection->name); ?> <?php echo esc_attr($bank_selection->details->account_number); ?> An. (<?php echo esc_attr($bank_selection->details->atas_nama); ?>)</span>
                     </label>
                 </li>
             <?php endforeach;
         endif; ?>
